...
class MenuHandler(BaseHandler):
    ...
    async def get_all(self) -> JSONResponse:
        cache_data = get_cache_data('get_menus')
        if cache_data:
            return json.loads(cache_data)
        repository = RepositoryFactory.create('menu', self.db)
        menus_list = await repository.get_all()
        create_cache_data(url='get_menus', time=60, object=json.dumps(menus_list))
        return JSONResponse(content=menus_list, status_code=status.HTTP_200_OK)
    
    async def get(self, menu_id: UUID) -> JSONResponse:
        cache_data = get_cache_data(f'get_menu_{menu_id}')
        if cache_data:
            return json.loads(cache_data)
        repository = RepositoryFactory.create('menu', self.db)
        menu = await repository.get(menu_id)
        if menu is not None:
            create_cache_data(url=f'get_menu_{menu_id}', time=60, object=JSONResponse(content=menu).body)
...
class SubMenuHandler(BaseHandler):
    ...
    async def get_all(self) -> JSONResponse:
        cache_data = get_cache_data('get_submenus')
        if cache_data:
            return json.loads(cache_data)
        repository = RepositoryFactory.create('submenu', self.db)
        submenus_list = await repository.get_all()
        create_cache_data(url='get_submenus', time=60, object=json.dumps(submenus_list))
    ...
    async def get(self, menu_id: UUID, submenu_id: UUID) -> JSONResponse:
        cache_data = get_cache_data(f'get_submenu_{submenu_id}')
        if cache_data:
            return json.loads(cache_data)
        repository = RepositoryFactory.create('submenu', self.db)
        submenu = await repository.get(menu_id=menu_id, id=submenu_id)
        if submenu is not None:
            create_cache_data(url=f'get_submenu_{submenu_id}', time=60, object=JSONResponse(content=submenu).body)
...
class DishHandler(BaseHandler):
    ...
    async def get_all(self) -> JSONResponse:
        cache_data = get_cache_data('get_dishes')
        if cache_data:
            return json.loads(cache_data)
        repository = RepositoryFactory.create('dish', self.db)
        dishes_list = await repository.get_all()
        create_cache_data(url='get_dishes', time=60, object=json.dumps(dishes_list))
    ...
    async def get(self, dish_id: UUID, submenu_id: UUID) -> JSONResponse:
        cache_data = get_cache_data(f'get_dish_{dish_id}')
        if cache_data:
            return json.loads(cache_data)
        repository = RepositoryFactory.create('dish', self.db)
        dish = await repository.get(dish_id=dish_id, submenu_id=submenu_id)
        if dish is not None:
            create_cache_data(url=f'get_dish_{dish_id}', time=60, object=JSONResponse(content=dish).body)
